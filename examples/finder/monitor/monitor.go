...
var msgReachMaxIdleCount = "[Monitor] The scheduler has been idle for a period of time (about %s) and reach the maxIdleCount (current: %d). Consider to stop it now."

// msgStopScheduler 代表停止调度器的消息模板。
var msgStopScheduler = "[Monitor] Stop scheduler...%s."
...
func Monitor(
	...
	logger.Infof("[Monitor] parameters: checkInterval: %s, summarizeInterval: %s, maxIdleCount: %d, autoStop: %v",
	...
	go func() {
		...
		for {
			...
			if currNumGoroutine != prevNumGoroutine ||
				...
				if err != nil {
					logger.Errorf("[Monitor] An error occurs when generating scheduler summary: %s\n", err)
					continue
				}
...
func reportError(
	...
	go func() {
		...
		for {
			...
			err, ok := <-errorChan
			if ok {
				errMsg := fmt.Sprintf("[Monitor] Received an error from error channel: %s", err)
				record(2, errMsg)
			}
		...
		for {
			...
			if scheduler.Idle() {
				...
				if idleCount >= maxIdleCount {
					msg := fmt.Sprintf(msgReachMaxIdleCount, time.Since(firstIdleTime).String(), idleCount)
					record(0, msg)
