...
func genResponseParsers() []module.ParseResponse {
	parseLink := func(httpResp *http.Response, respDepth uint32) ([]module.Data, []error) {
		...
		// 检查响应。
		if httpResp == nil {
			return nil, []error{fmt.Errorf("[ParseResponse - parseLink] nil HTTP response")}
		}
		httpReq := httpResp.Request
		if httpReq == nil {
			return nil, []error{fmt.Errorf("[ParseResponse - parseLink] nil HTTP request")}
		}
		reqURL := httpReq.URL
		if httpResp.StatusCode != 200 {
			err := fmt.Errorf("[ParseResponse - parseLink] unsupported status code %d (requestURL: %s)", httpResp.StatusCode, reqURL)
			return nil, []error{err}
		}
		body := httpResp.Body
		if body == nil {
			err := fmt.Errorf("[ParseResponse - parseLink] nil HTTP response body (requestURL: %s)", reqURL)
			return nil, []error{err}
		}
		// 检查HTTP响应头中的内容类型。
		...
		doc.Find("a").Each(func(index int, sel *goquery.Selection) {
			...
			if err != nil {
				logger.Warnf("[ParseResponse - parseLink] An error occurs when parsing attribute %q in tag %q : %s (href: %s)", err, "href", "a", href)
				return
			}
			if !aURL.IsAbs() {
	...
	parseImg := func(httpResp *http.Response, respDepth uint32) ([]module.Data, []error) {
		// 检查响应。
		if httpResp == nil {
			return nil, []error{fmt.Errorf("[ParseResponse - parseImg] nil HTTP response")}
		}
		httpReq := httpResp.Request
		if httpReq == nil {
			return nil, []error{fmt.Errorf("[ParseResponse - parseImg] nil HTTP request")}
		}
		reqURL := httpReq.URL
		if httpResp.StatusCode != 200 {
			err := fmt.Errorf("[ParseResponse - parseImg] unsupported status code %d (requestURL: %s)", httpResp.StatusCode, reqURL)
			return nil, []error{err}
		}
		httpRespBody := httpResp.Body
		if httpRespBody == nil {
			err := fmt.Errorf("[ParseResponse - parseImg] nil HTTP response body (requestURL: %s)", reqURL)
			return nil, []error{err}
		}
		// 检查HTTP响应头中的内容类型。
		dataList := make([]module.Data, 0)
